...
export default function App() {
  ...
  const canStart = useMemo(
    () => pool.length >= 1 && name.trim().length > 0,
    [pool.length, name]
  );
  ...
  if (step === "start") {
    content = (
      <div style={wrapStyle}>
        ...
        <p style={{ opacity: 0.8, marginBottom: 16 }}>名前を入力してスタート</p>
    ...
    content = (
      <div style={wrapStyle}>
        ...
        <div style={{ marginBottom: 8 }}>
          名前：<b>{name}</b> ／ 形式：{MODE_FIXED} ／ 難易度：{DIFF_FIXED}
        </div>
